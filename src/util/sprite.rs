...
pub fn load_aseprite_sprites(raw_json: &str, texture: TextureId) -> anyhow::Result<LoadedSprites> {
    ...
    let sprites_and_tags = tags.iter().map(|tag| {
        ...
        let uv_topleft = (
            glam::dvec2(first_frame.x as f64, first_frame.y as f64) / texture_dims
        ).as_vec2();
